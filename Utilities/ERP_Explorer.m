...
% Last Modified by GUIDE v2.5 31-May-2019 13:36:07
...
handles.PlotByButtonGroup.SelectedObject = handles.AmpRadioButton;
handles.PlotEditText1.String = '1';
handles.PlotEditText2.String = '1';
for i = 1:length(handles.Data.Info.Amplitudes_mA) %10
...
for i = 1:19%length(handles.Data.Info.Amplitudes_mA) %11
    if (i <= size(latdata,2))
...
for i = 1:length(l)
    ...
    if ((size(basedata,2) > 1) && any(basedata(:,i))) % only plot marker if one has been marked
...
switch (hObject.String)
    case 'Amplitudes'
        ...
        numboxes = size(handles.Data.ERPs, 4);
    case 'Channels'
        handles.PlotText1.String = 'Condition:';
        handles.PlotText2.String = 'Amplitude:';
        numboxes = size(handles.Data.ERPs, 2);
    case 'Conditions'
        handles.PlotText1.String = 'Amplitude:';
        handles.PlotText2.String = 'Channel:';
        numboxes = size(handles.Data.ERPs, 1);
end
...
%         handles.Data.Peaks(i).ECoG(j).Group = changem(handles.Data.Peaks(i).ECoG(j).Group, newgroupnums, groupnums);
        [bool, idx] = ismember(handles.Data.Peaks(i).ECoG(j).Group, groupnums);
        handles.Data.Peaks(i).ECoG(j).Group = newgroupnums(idx(bool));
        if (isempty(handles.Data.Peaks(i).ECoG(j).Group))
            handles.Data.Peaks(i).ECoG(j).Group = NaN;
        end
    end
end
