...
%%%     - saveonly: if true, save data file but don't parse
%%%
%%% Output:
...
function E = ParseEEGDatav2(fname, paradigm, ttlchan, saveonly, stimfname, stimfpath)
...
[EEG, hdr] = ImportBV(eegfpath, eegfname, saveflag);
...
% If only saving data file, return now:
if (saveonly)
    E = [];
    return;
end
...
events = cellfun(@(x) str2double(x(end-1:end)), {EEG.event(2:end).type}); % all marker numbers
trigeventidxs = find(~isnan(events)); % find events corresponding to TTL inputs
eventsbin = dec2bin(events(trigeventidxs)) == '1'; % convert to binary
trigidxs = find(~all(eventsbin,1)); % which trigger outputs were used for the TTL channel(s)
trig = trigidxs(ttlchan); % which trigger are we syncing to
risingedgeidx = trigeventidxs(find(diff([0;eventsbin(:,trig)]) > 0)) + 1;
fallingedgeidx = trigeventidxs(find(diff(eventsbin(:,trig)) < 0) + 1) + 1;
fprintf('    - Using trigger %d to sync\n', trig);
