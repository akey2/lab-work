...
function [EEG, hdr, com] = ImportBV(path, hdrfile, readdata)
...
if isfield(hdr, 'comment')
    idx1 = find(cellfun(@(x) startsWith(x, 'Impedance'), hdr.comment));
    idx2 = find(cellfun(@(x) startsWith(x, 'GND'), hdr.comment));
    if (isempty(idx2))
        idx2 = length(hdr.comment);
    end
    z = reshape(char(hdr.comment(idx1:idx2))', 1, []);
    x = regexp(z, cellfun(@(x) [x, ': (\S+)'], {EEG.chanlocs.labels}, 'uni', 0), 'tokens', 'once');
    x(cellfun(@isempty, x)) = {'nan'};
    EEG.chanzs = str2double([x{:}]);
    EEG.rectime = char(regexp(z, 'recording started at ([0-9:]+)', 'tokens', 'once'));
    EEG.ztime = char(regexp(z, 'Impedance \[KOhm\] at ([0-9:]+)', 'tokens', 'once'));
end
...
% Test and make sure we actually opened the file - this is necessary for
% loading files directly from Box Drive.
IN = fopen(fullfile(path, hdr.commoninfos.datafile));
fread(IN, 10);
if (ftell(IN) == 0)
    fclose(IN);
    IN = fopen(fullfile(path, hdr.commoninfos.datafile));
